  ...
  return (
    <div className=' bg-[#030712]'>
      <div className="min-h-full">
                  ...
                  <nav className="ml-10 flex items-baseline space-x-4">
                    {navigation.map((item) => (
                      <NavLink
                        key={item.name}
                        to={item.to}
                        className={({ isActive }) => isActive ? 'bg-gray-950/50 text-white rounded-md px-3 py-2' : 'text-gray-300 hover:bg-white/5 hover:text-white rounded-md px-3 py-2 text-sm font-medium'}
                      >
                        {item.name}
                      </NavLink>
                    ))}
            ...
            <nav className="space-y-1 px-2 pt-2 pb-3 sm:px-3">
              {navigation.map((item) => (
                <NavLink
                  key={item.name}
                  to={item.to}
                  className={({ isActive }) => isActive ? 'bg-gray-950/50 text-white' : 'text-gray-300 hover:bg-white/5 hover:text-white rounded-md px-3 py-2 text-sm font-medium'}
                >
                  {item.name}
                </NavLink>
              ))}
        ...
        {Object.keys(params).length === 0 && <header className="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:inset-y-0 after:border-y after:border-white/10">
        ...
        <main>
          <div className="relative mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 min-h-screen">
            <Outlet />
          </div>
        </main>
      </div>
